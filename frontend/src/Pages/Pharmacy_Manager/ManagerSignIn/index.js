...
const ManagerSignIn = () => {
    ...
    const signInManager = () => {
        if(email.length === 0)
            ...
            const url = `${URL}/pharmacymanager/signin`
            axios.post(url, body).then((response) => {
                const result = response.data

                if(result.status === "SUCCESS") {
                    toast.success("Welcome to mediquick")
                    const { id, name, email, pharmacyId, pharmacyName } = result.data;

                    sessionStorage['id'] = id;
                    sessionStorage['name'] = name;
                    sessionStorage['email'] = email;
                    sessionStorage['pharmacyId'] = pharmacyId;
                    sessionStorage['pharmacyName'] = pharmacyName;
